# template.py
#
# Patched chat template for google/functiongemma-270m-it.
# Injected into the tokenizer at load time by load_model_and_tokenizer()
# in utils.py, overriding the template bundled in tokenizer_config.json.
#
# Fixes vs. the original HuggingFace template:
#   - All `| dictsort` replaced with `.items()` — preserves Python dict
#     insertion order so parameter ordering matches your tool schema exactly.
#   - Property field order: type → description → (type-specific) → nullable
#     matches the golden prompt expected by the literlm Android engine.
#   - Parameters block order: type → properties → required.
#   - ARRAY items block uses explicit type → properties → required sequencing
#     (not a loop) so ordering is guaranteed regardless of dict construction.
#   - `description` is guarded with `is defined` for schemas that omit it.
#   - `nullable` emitted after type-specific content, before closing `}`.
#
# Uses r"" raw string so the Jinja template is readable as-is with no
# Python escape noise.

FUNCTIONGEMMA_CHAT_TEMPLATE = r"""{%- macro format_parameters(properties, required) -%}
    {%- set ns = namespace(found_first=false) -%}
    {%- for key, value in properties.items() -%}
        {%- if ns.found_first %},{% endif -%}
        {%- set ns.found_first = true -%}
        {{- key -}}:{type:<escape>{{ value['type'] | upper }}<escape>
        {%- if value['description'] is defined -%}
            ,description:<escape>{{ value['description'] }}<escape>
        {%- endif -%}
        ...
        {%- elif value['type'] | upper == 'ARRAY' -%}
            {%- if value['items'] is mapping and value['items'] -%}
                ,items:{
                {# Explicit type -> properties -> required — not a loop, so order is
                   guaranteed regardless of how the items dict was constructed. #}
                {%- set ns_items = namespace(found_first=false) -%}
                {%- if value['items']['type'] is defined -%}
                    {%- if value['items']['type'] is string -%}
                        type:{{ format_argument(value['items']['type'] | upper) }}
                    {%- else -%}
                        type:{{ format_argument(value['items']['type'] | map('upper') | list) }}
                    {%- endif -%}
                    {%- set ns_items.found_first = true -%}
                {%- endif -%}
                {%- if value['items']['properties'] is defined and value['items']['properties'] is mapping -%}
                    {%- if ns_items.found_first %},{% endif -%}
                    {%- set ns_items.found_first = true -%}
                    properties:{
                    {{- format_parameters(value['items']['properties'], value['items']['required'] | default([])) -}}
                    }
                {%- endif -%}
                {%- if value['items']['required'] is defined -%}
                    {%- if ns_items.found_first %},{% endif -%}
                    required:[
                    {%- for req_item in value['items']['required'] -%}
